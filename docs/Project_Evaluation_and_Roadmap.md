...
### 2.4 版本发布 ✅
- [x] **v1.0.250 发布**: 将核心逻辑与 UI 修复打包发布，并正确打标推送到 GitHub。
...
| **修复：文件信息面板 (FileInfoPanel)** | 修正了主/副面板底部的 `FileInfoPanel` 样式资源引用错误 | ✅ 已修复 |
...
*   **注意**：不包含基本文件信息（大小、修改日期等），那是主/副 `FileInfoPanel` 的职责。

| 步骤 | 任务描述 | 验证方法 (可测试性) | 状态 |
|------|----------|-------------------|------|
| **1.1** | **创建 RightPanelViewModel** | 在 `ViewModels` 目录创建类并注册 DI，负责预览和备注状态 | 检查 DI 容器能否成功解析该 VM | ✅ 已完成 |
| **1.2** | **移除冗余信息区** | 从 XAML 和 ViewModel 中移除基本文件信息的显示逻辑（这些属于 `FileInfoPanel`） | 确认右侧面板只剩预览和备注区 | ✅ 已完成 |
| **1.3** | **UI 绑定与迁移** | 将预览图路径、Webview内容、备注文本绑定到 `RightPanelViewModel` | 实现选中图片即在右侧显示预览，选中任意文件可编辑备注 | ⏳ 下一步 |
| **1.4** | **清理 MainWindow 后置代码** | 移除旧的 `RightPanel` 操作代码，防止干涉 VM | 代码清爽，无 UI 直接操作逻辑 | ⚪ 计划中 |
