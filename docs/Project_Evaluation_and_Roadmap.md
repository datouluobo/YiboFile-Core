...
| **BUG-002** | **Logic/Pane** | **副面板刷新与操作混乱** ✅确认: 副面板进行删除/粘贴操作时常导致主面板刷新，或副面板自身不刷新；F5 刷新对副面板无效。根源在于操作上下文（Context）仍重度依赖 WPF 焦点而非 VM 激活状态。**已在v1.0.330后测试确认，待 MVVM 彻底解耦后修复**。 | ⏳ 待 MVVM 彻底解耦后修复 |
| **BUG-003** | **Logic/Library** | **副面板库路径识别失败** ✅确认: 在库模式下，副面板的文件操作无法正确解析 `lib://` 协议，导致路径指向错误。需统一 FileOperationModule 的路径解析逻辑。**已在v1.0.330后测试确认，待 MVVM 彻底解耦后修复**。 | ⏳ 待 MVVM 彻底解耦后修复 |
| **BUG-004** | **UI/FileList** | **副列表库加载失效**: 使用侧边导航栏打开“库”时，副文件列表显示为空。切换标签页后恢复。原因在于侧边栏导航消息未能正确触达非活跃面板的 VM 更新流。 | ⏳ 待 MVVM 迁移完成修复 |
| **BUG-005** | **Filter/Secondary** | **副列表过滤器不可用**: 副面板的文件过滤功能部分失效，无法应用按文件过滤条件。PaneViewModel 的过滤逻辑未能正确绑定到副面板的 UI。 | ⏳ 待修复 |
---| **BUG-006** | **Filter/FileSize** | **尺寸过滤器未排除文件夹**: 使用文件大小过滤器时，文件夹仍然显示在结果中。应该只显示文件。 | ⏳ 待修复 |
---
---
...
| **BUG-005** | **Filter/Secondary** | **副列表过滤器不可用**: 副面板的文件过滤功能完全失效，无法应用任何过滤条件。PaneViewModel 的过滤逻辑未能正确绑定到副面板的 UI。 | ⏳ 待修复 |
| **BUG-006** | **Filter/FileSize** | **尺寸过滤器未排除文件夹**: 使用文件大小过滤器时，文件夹仍然显示在结果中。应该只显示文件。 | ⏳ 待修复 |
