...
> ⚠️ **重要提示**: v1.5.2 和 v1.5.3 版本存在问题，不推荐使用。请升级到 [v1.5.4](../../releases/tag/v1.5.4) 或更高版本。

### v1.5.4 (2025-12-20)
**库管理代码拆分与重构**
- ♻️ 完成库管理功能拆分到独立文件 `MainWindow.Library.cs`
- 🗑️ 删除重复代码，净减少 ~130 行
- 📂 代码组织更清晰，职责分离更明确
- 🔧 修复命名空间引用问题
- ✅ 解决了库管理相关的 9 个编译错误

### v1.5.3 (2025-12-20) ⚠️ **不推荐使用**
**地址栏重构与界面优化**
- ✨ 重构地址栏显示方式：从按钮式改为文本式显示，更简洁流畅
- 🎨 长路径智能优化：超过10段时左右分开显示，右侧右对齐
- ✅ 恢复搜索结果分组显示功能
- ⌨️ 新增 Alt+D 快捷键快速访问地址栏
- 📝 优化备注区域控制：根据选择状态自动启用/禁用
- 🔄 恢复搜索标签页切换刷新功能
- ⚠️ **已知问题**: 库管理代码拆分未完成，存在编译错误，请使用 v1.5.4

### v1.5.2 (2025-12-20) ⚠️ **已弃用**
**运行时错误修复**
- ✅ 修复程序启动时的 ArgumentException 错误
- ✅ 移除无效的事件绑定（NavigationBack/Forward/Up）
- ✅ 清理未定义的事件处理器引用
- ⚠️ **已知问题**: 地址栏显示功能不完整，请使用 v1.5.4
...
- ✅ 修复排序方向双重触发问题
- ✅ 修复日期字段按字符串排序问题
- ✅ 整合ColumnService和ColumnHeaderService
- ✅ 添加防抖机制防止重复点击
- ✅ 代码优化：净减少800行代码
- ✅ 清理编译警告
